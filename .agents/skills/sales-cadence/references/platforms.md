...
- [In AiSDR](#in-aisdr)
- [In 11x.ai (Alice)](#in-11xai-alice)
- [In Nooks](#in-nooks)
- [In Orum](#in-orum)
...
- Alice is an AI SDR that handles multi-channel outbound across email, LinkedIn, SMS, and WhatsApp — fully autonomous sequence execution
- AI-personalized outreach: Alice researches each prospect (company news, tech stack, job postings, signals) and writes unique messages. Quality depends heavily on AI Strategist context input
- Channels: email + LinkedIn + SMS + WhatsApp. No outbound phone dialer — Alice doesn't make calls. For phone, pair with Orum, Nooks, or ConnectAndSell
- 400M+ verified contact database with real-time search and signal detection (funding, hiring, tech stack changes)
- Smart Reply: AI reads prospect replies, analyzes tone, and generates contextual follow-ups
- Sequence builder with playbook support, but less granular control than traditional tools (Salesloft, Outreach)
- Lead routing: qualified leads automatically routed to the right rep
- CRM sync: Salesforce, HubSpot, Zoho via Ampersand OAuth — all activity logs bidirectionally
- Meeting booking: built-in scheduling (within 20 seconds of form submission via phone agent)
- Pricing: ~$5,000/mo for ~3,000 contacts, annual commitment required. ~$1.67 per contact — significantly more expensive than AiSDR ($0.75/message) or Salesforge Agent Frank ($0.50/contact)
- Controversy: reported gross retention below 50%, TechCrunch investigation into metrics, users report generic personalization despite claims
- Best for: enterprise teams with budget who want autonomous multi-channel outbound including inbound phone handling. Trade-off: highest price point, annual lock-in, mixed reliability reports
- Platform skill: `/sales-11x`
...
- **AI-built sequences**: Swan builds LinkedIn + email outreach sequences from natural language prompts. Describe your target audience and messaging intent — Swan drafts personalized messages and executes multi-channel cadences autonomously.
- **Owly agent (LinkedIn intent outbound)**: Detects LinkedIn engagement signals → enriches profiles → researches context → drafts personalized connection requests and messages → sends automatically.
- **Multi-channel execution**: Swan handles both LinkedIn connection requests/messages and email sequences within the same workflow. No need for separate sequence tools.
- **Credit cost**: Each outreach action (message send, connection request) costs 1 credit. A 5-touch sequence to 100 prospects = ~500 credits.
- **Limitations**: Not a dedicated cold outreach tool — no email warmup, no mailbox rotation, no deliverability monitoring. For high-volume cold email (1,000+/day), use Smartlead or Lemlist. Swan is best for personalized, lower-volume sequences tied to intent signals.
- **Best for**: Teams wanting outreach triggered by intent signals (website visits, LinkedIn engagement) with AI-personalized messaging. Not for bulk cold outreach.
- **Platform skill**: `/sales-swan`
