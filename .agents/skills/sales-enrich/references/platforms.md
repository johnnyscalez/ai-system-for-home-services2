...
- **AI-powered enrichment**: Swan enriches contacts and accounts as part of autonomous GTM workflows. Describe what you need in plain English — Swan handles the data fetching using its own aggregation layer across multiple providers.
- **API enrichment**: `POST /research/person` endpoint enriches individuals with email, phone, job title, LinkedIn URL, company info, and job start date. `POST /research/company` answers questions about companies with confidence scoring (1-10). Base URL: `https://api.orchestra.inc/v1`, auth via `x-api-key` header.
- **Credit-based**: Each enrichment action costs 1 credit ($240/mo for 4,000 credits). No per-contact pricing — same cost whether enriching 1 field or all fields.
- **Aggregated sources**: Swan pulls from multiple data providers (Apollo, ZoomInfo, etc.) through its own layer. No need for separate subscriptions to individual providers.
- **Best for**: Teams wanting enrichment as part of an end-to-end GTM workflow (research → qualify → enrich → outreach) rather than standalone data lookups. For granular waterfall enrichment across many providers, Clay offers more control.
