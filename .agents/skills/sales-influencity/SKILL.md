...
description: "Influencity platform help — all-in-one influencer marketing with Discover (200M+ profiles on Instagram, TikTok, YouTube), IRM (relationship management, audience analysis, fake follower detection), Campaigns (Shopify gifting, coupon tracking, bulk email outreach), Reports (PDF/Excel export), Social Hub (content calendar, approvals), Monitoring (brand and competitor tracking), Analytics, Content Planner, and Influencer Payments, plus a credit system (Monthly Results/Analyses/Post Analyses). Use when Influencity campaigns aren't delivering, discovery returns irrelevant creators, IRM data is messy, reporting doesn't match performance, Shopify gifting isn't tracking, Social Hub posts aren't publishing, Monitoring misses mentions, or unsure which plan to choose. Do NOT use for influencer strategy across platforms (use /sales-influencer-marketing), ad campaign strategy (use /sales-retargeting), email marketing to subscribers (use /sales-email-marketing), or affiliate program design (use /sales-affiliate-program)."
