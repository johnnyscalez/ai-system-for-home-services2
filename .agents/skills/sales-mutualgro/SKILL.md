...
**Skill does**: Confirms MutualGro has no API/webhooks/Zapier. Walks through UTM tagging on the swap link (`?utm_source=mutualgro&utm_medium=crosspromo&utm_campaign={partner-slug}`), enabling Beehiiv's UTM-based subscriber tagging, then comparing 30-day open rate vs organic to measure swap quality. Notes the workflow stays manual at the partner-level — there's no programmatic pipeline.
