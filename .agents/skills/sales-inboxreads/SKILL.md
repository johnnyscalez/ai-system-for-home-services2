...
description: "InboxReads platform help — newsletter directory and growth platform with 5,600+ newsletters across 40+ topics, 1-click cross-promotion swaps, an Opportunities Board for sponsorships, Live Media Kits, ad pricing suggestions, and a newsletter acquisition marketplace. Use when comparing InboxReads tiers (Free vs Lite $8/mo vs Basic $33/mo vs Team $58/mo) to find which unlocks the analytics and messaging you need, can't decide between InboxReads and Lettergrowth/SparkLoop/Paved for cross-promotion or sponsor discovery, setting up a Live Media Kit and unsure which metrics to feature, the Opportunities Board isn't surfacing relevant matches, hit the message or search caps on Lite, looking for newsletters to acquire or to list yours for sale, or piping InboxReads data into a CRM despite no API. Do NOT use for general newsletter monetization strategy across all platforms (use /sales-newsletter) or general newsletter audience growth strategy (use /sales-audience-growth)."
...
**Skill does**: Confirms InboxReads has no API/webhooks/Zapier. Walks through the Basic+ CSV export path: filter the directory by niche/size, export 5 contacts/month (Team = unlimited), map columns to HubSpot fields (newsletter name → company, contact email → email, subscriber count → custom property), then run outreach via a regular tool like Mailshake or Smartlead. Warns that many directory entries don't expose a contact email — run those rows through an email finder (Hunter, Anymail Finder) before the HubSpot import. Notes the manual nature and recommends batching exports.
