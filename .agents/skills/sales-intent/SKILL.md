...
### In Swan (website visitor ID + intent)
- **Gatto agent**: Deanonymizes website visitors → qualifies against ICP → routes to reps → launches personalized outreach. Autonomous workflow from anonymous visit to qualified lead in minutes.
- **Owly agent**: Monitors LinkedIn engagement signals (profile views, post interactions) as intent triggers for outbound sequences.
- **Multi-signal**: Swan combines website visits, LinkedIn engagement, and CRM data to prioritize leads. Natural language ICP definitions control which signals matter.
- **Exclusion API**: `POST /exclusion/company` filters out known companies (competitors, partners) from visitor ID — up to 1,000 domains per call.
- **Limitations**: Swan's visitor ID relies on connected tools (RB2B, etc.) — it's an orchestration layer, not a standalone identification provider. Intent monitoring covers website and LinkedIn only — no third-party intent feeds (unlike Bombora/6sense).
- **Best for**: Teams wanting autonomous intent-to-outreach workflows without manual lead routing.
