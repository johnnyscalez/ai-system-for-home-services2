...
- [In Moosend](#in-moosend)
- [In Sender](#in-sender)
- [In Constant Contact](#in-constant-contact)
...
Moosend is a budget ESP with unlimited sends on all plans, visual automation workflows, and AI-powered ecommerce product recommendations. Acquired by Sitecore in 2021. Pro starts at $7/mo (annual) for 500 subscribers.

**Campaigns**
- Drag-and-drop editor with responsive templates.
- A/B testing for subject line, content, and sender name.
- Scheduling with timezone-based delivery.
- Campaign-level analytics: opens, clicks, bounces, unsubscribes, complaints.

**Automation**
- Visual workflow builder with 30+ pre-built recipes (welcome, cart abandonment, re-engagement, birthday, etc.).
- Triggers: subscription, email opened/clicked, link clicked, cart abandoned, product purchased, date-based, custom event.
- Conditional splits based on subscriber data, behavior, or segment membership.
- Automation triggers cannot be changed after creation — must recreate for a different trigger.

**Segmentation**
- Behavioral segments (opened campaign X, clicked link Y, purchased product Z).
- Demographic segments (custom fields, location).
- Conditional segments update in near-real-time.
- Available via API (`/v3/lists/{id}/segments`).

**Landing pages & forms** (Pro+ only)
- Drag-and-drop landing page builder with templates.
- Subscription forms: pop-up, inline, floating bar, full-page.
- Custom domains for landing pages.

**Ecommerce AI recommendations**
- Product recommendation engine with cross-sell, upsell, weather-based, and recently viewed algorithms.
- Requires Moosend website tracking JS installed on your store.
- Native integrations: Shopify, WooCommerce, Magento, BigCommerce, PrestaShop, OpenCart.

**API** (all plans including trial)
- REST v3 at `https://api.moosend.com/v3/`, API key as query parameter.
- ~50 endpoints: Campaigns (20), Mailing Lists (10), Segments (8), Subscribers (12).
- Page-based pagination (1-indexed, max 500 per page).
- SDKs: PHP, Java, .NET (no official Python/JS).
- Zapier triggers + actions available.

**Pricing**
- No free plan — 30-day trial only.
- Pro: $7/mo annual ($9/mo monthly) for 500 subs, scales by list size.
- Unlimited sends on all plans.
- Dedicated IP requires Moosend+ (custom pricing).

**Best for**: Solopreneurs and small teams who need affordable unlimited-send email marketing with solid automation and ecommerce features. Less powerful than ActiveCampaign or Klaviyo for complex multi-channel automation, but significantly cheaper for basic email marketing workflows.

### In Sender

Sender (sender.net) is a budget ESP with a genuine free tier (2,500 subscribers, 15K emails/mo) and premium features included at no cost.

**Campaigns**
- Drag-and-drop editor with 1,600+ responsive templates.
- A/B testing for subject lines, content, sender name, and send time.
- HTML builder for custom designs.
- Auto-followup: automatically resend to non-openers after 12-168 hours.

**Automation**
- Visual workflow builder with pre-made sequences.
- Unlimited automation workflows on ALL plans including free.
- Triggers: form submission, group membership change, custom events via API.

**SMS**
- SMS campaigns integrated with email workflows (Standard+ plan).

**Landing pages & forms**
- Drag-and-drop landing page builder (included on free plan).
- Signup forms: popup, inline, floating bar.

**Transactional email**
- Template-based or raw HTML sending via API.
- Included on free plan.

**Segmentation**
- Basic filters: opened, clicked, subscribed date, custom field values.
- No advanced behavioral logic or real-time triggers (unlike ActiveCampaign/Klaviyo).

**API** (all plans including free)
- REST v2 at `https://api.sender.net/v2/`, Bearer token auth.
- ~61 endpoints: Subscribers (9), Groups (6), Segments (4), Fields (4), Campaigns (10), Transactional (7), Statistics (8), Workflows (6), Custom Events (1), Webhooks (6).
- Webhooks: paid plans only, subscriber events (new-subscriber, unsubscribed).
- Zapier triggers + actions available.

**Pricing**
- Free Forever: 2,500 subs, 15K emails/mo, includes automation + landing pages + transactional.
- Standard: from $7/mo, no branding, SMS, A/B testing.
- Professional: from ~$15/mo, dedicated IPs (20K+ subs), advanced reports, priority support.
- Enterprise: custom (SSO, SLA, phone support).

**Gotchas**
- No deliverability dashboard — use external tools for inbox placement visibility.
- Free plan includes Sender branding in footer.
- Webhooks are paid-only (Standard+).
- Segmentation is basic — no advanced behavioral triggers.

**Best for**: Solopreneurs and small teams who want a generous free tier with automation included. More features on the free plan than Mailchimp or MailerLite. Less powerful segmentation and automation logic than ActiveCampaign or Klaviyo, but significantly cheaper for straightforward email marketing.

### In Constant Contact

Constant Contact (est. 1995) is a well-known ESP targeting small businesses and nonprofits. Its unique differentiator is built-in event management with ticket sales, registrations, and check-ins — a feature most competitors lack. V3 REST API (OAuth 2.0) covers contacts, campaigns, events, and reporting. Deliverability ranks 2nd in independent tests (88-90%). Overpriced compared to newer competitors with relatively basic automation.

**Key email marketing features**
- Drag-and-drop email builder with ~200 templates
- A/B testing on subject lines and content (Standard+ only)
- Contact segmentation by behavior and demographics (Standard+ only)
- Pre-built automations: welcome, birthday, anniversary, click-based
- Custom automation paths (Premium only — $80/mo+)
- Social posting (Facebook, Instagram, LinkedIn) integrated with campaigns
- Event management: registrations, ticket sales (PayPal/Stripe), check-ins (5.4% + $0.80 per transaction)
- SMS campaigns (US only, Premium only)
- 300+ integrations (Shopify, WooCommerce, WordPress, Salesforce, Canva)

**Pricing**
- Lite: $12/mo (500 contacts), 1 user, basic campaigns, 1 welcome automation.
- Standard: $35/mo (500 contacts), 3 users, A/B testing, segmentation, pre-built automations.
- Premium: $80/mo (500 contacts), unlimited users, custom automation, SMS, dynamic content.
- No free plan. Annual prepayment saves 15%.

**Gotchas**
- All contacts count toward billing — including unsubscribed and bounced. Must manually delete to reduce bill.
- No free plan — MailerLite and Sender offer comparable features free.
- Cancellation requires a phone call during Eastern business hours.
- Automation is basic on Lite/Standard — no conditional logic or branching.
- No dedicated IPs, no warm-up guidance, no deliverability dashboard.

**Best for**: Small businesses already on Constant Contact who use event management for ticket sales and registrations. If you don't need events, newer ESPs (MailerLite, Sender, Brevo) offer better value with stronger automation at lower prices. For `/sales-constantcontact` platform help.
