...
- **ICP-based qualification**: Swan qualifies leads against ICP criteria as part of autonomous workflows. Define your ICP in natural language — Swan researches, enriches, and scores leads automatically.
- **Gatto agent (website visitors)**: Deanonymizes visitors → qualifies against ICP → routes qualified leads to reps → launches personalized outreach. Qualification happens autonomously based on your ICP definition.
- **Contextual scoring**: Swan's AI agents reason about lead quality using enriched data, intent signals, and your ICP rules. Not a traditional point-based scoring model — more like an AI analyst reviewing each lead.
- **Limitations**: No configurable point-based scoring model. No score decay or threshold automation. Qualification is binary (fits ICP or doesn't) rather than graduated. For granular scoring with weights and thresholds, use HubSpot, Salesforce, or ActiveCampaign's built-in scoring.
- **Best for**: Teams that want AI-driven lead qualification without building manual scoring models. Good for qualifying inbound leads and website visitors before they reach sales.
