...
### Swan integrations
- **Native CRM**: HubSpot (deepest — creates/updates contacts, companies, deals automatically), Salesforce, Pipedrive, Attio.
- **Sales engagement**: Salesloft, Outreach — push enriched contacts into sequences.
- **Data providers**: Apollo, ZoomInfo, 6sense, RB2B — Swan aggregates enrichment from multiple providers via its own layer.
- **Communication**: LinkedIn (outbound sequences), Email (sequences), Slack (copilot + real-time notifications).
- **Other**: Gong, Marketo, ActiveCampaign.
- **API**: REST API at `api.orchestra.inc/v1` — 3 endpoints: research company, research person, exclude company. `x-api-key` header auth.
- **No webhooks, no Zapier/Make**: All integrations are native. Workflow orchestration happens inside Swan — no middleware needed.
- **Best for**: Teams that want to connect enrichment, CRM, and outreach in a single autonomous workflow rather than stitching together Zapier/Make automations.
