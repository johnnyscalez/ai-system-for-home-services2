...
- ☐ **A "stopped" Messenger agent can still SPEAK — now gated in code.** The questionnaire opener rides the `leadgen` webhook and sending needs no `messages` subscription, so a naive stop leaves the mouth on with the ears off (live: 3 hot leads waited 13–16h unanswered after saying yes). Since Aug 2026 the opener checks the page subscription first: **no `messages` field = no opener** — the lead lands paused, the office gets pinged to reply from the page inbox within Meta's 24h window, and no SMS fallback fires. The subscription is the single on/off source of truth. Stopping = strip subscription to `leadgen` + pause non-terminal Messenger leads (NEVER booked leads — pausing them blocks their confirmation/reminder texts). Re-enabling = restore all five fields (verify `message_echoes` by read-back) + unpause.
...
- **THE AGREED-TIME CONTRACT (P-1..P-3, Aug 12 2026, the KTrin lesson):** the booked/rescheduled ISO is NEVER the model's own datetime. `resolveAgreedSlot` is the single authority on every channel — exact match to a tool-offered slot, else the slot the THREAD's own words name (window labels like "3 to 6pm" parse to their start; narrowed by month-day → weekday → the model's calendar day), else same-day-unique, else the booking is REFUSED with a re-ask (SMS/Messenger; voice proceeds and flags — dead air is worse on a call). A reschedule that resolves to the CURRENT time is a self-move tell: re-resolve against the other offered slots. Live evidence from the battery: the real model composed the datetime WRONG in 4 of 5 runs (20:00Z, 00:00Z = KTrin's exact 8 PM, 23:00Z, and a self-move) — every one corrected to the agreed slot. Corollary: every bookable value is slot-engine output, so AI bookings can only land on configured arrival-window starts on bookable days.
- **Business-hours bound (P-2, closes the oddball-hour hole):** a time outside every enabled arrival window is rejected by `techCanTakeBooking` whenever windows are configured — an anchored day plus no ±2h conflict no longer lets an invented 8 PM through. Tool-offered escape only; window-less accounts keep legacy ±2h behavior.
- **FULL NAME FOR THE BOOKING (Q-1, the KTrin Brownskin lesson):** Facebook hands over PROFILE names (form autofill AND Messenger PSID profile) — often aliases, never verified. Both FB paths stamp `metadata.name_source="facebook_profile"`; a visit booking is HELD (deterministic corrective "Perfect. And what's your full name for the booking?") until the lead has a name that is not an unverified FB alias. A name saved via `update_lead_details` counts as customer-stated ONLY when it appears in the customer's own message that turn (`name_verified` + `name_source="customer_stated"`) — the model cannot "verify" the alias it sees on file. The HCP customer is created from the verified name. Greeting by the profile first name in chat stays correct — it's the booking identity that must be real.
...
| K-3b | The Messenger nurture SHIPPED but could never FIRE: the cron's 24h-window check reads `leads.last_inbound_at`, and the Facebook webhook was the one inbound path that never wrote it — NULL read as "window closed", every messenger step fell to SMS, placeholder phones made those ghosts (a "Not delivered" bubble the lead never received — Olya, followed up 7h into an open window) | FB webhook stamps `last_inbound_at` + `is_active_conversation` on every inbound like all other channels; 79 leads backfilled from conversation history; the cron decides TRANSPORT BEFORE GENERATING (unreachable step = silent cancel, zero ghosts; post-generation transport failure deletes the ghost row). **Law: when a feature keys off a denormalized field, verify EVERY channel writes it — and test the FIRING, not just the arming** | Battery item: fire one real cron step for a messenger lead inside the window → message arrives ON Messenger, `channel='messenger'` + message id on the row. |
...
### P. The composed datetime (the KTrin clock class — added Aug 12, 2026)

KTrin agreed to "Tuesday, Aug 18, 3 to 6pm" through a flawless Messenger flow — tenure question, $89→$189 mechanism, consent recap, her "Yes." The model then called `book_appointment` with a datetime it COMPOSED itself instead of copying the slot tool's ISO, and the composition resolved to **8:00 PM her time**. Every guard let it through, each for its own correct-sounding reason: the offered-slot snap gate was scoped to sales bookings only; the datetime sanity gate only rejects past/absurd dates; the tech audit's outside-every-window branch checked ±2h conflicts + the anchor day but had NO business-hours bound, and Alex K had Aug 18 work. The 8 PM job pushed to HCP; the office dragged it back to 3–6 by hand that night. Two strokes of luck contained it: the "8:00 PM" confirmation SMS died at the carrier (unregistered A2P), and no Messenger message ever showed a clock time — she never saw a wrong hour.

The kicker from the fix battery: driving the REAL model through clean consent turns, it composed the datetime **wrong in 4 of 5 runs** — 20:00Z, 00:00Z (KTrin's exact value), 23:00Z, and a reschedule aimed at the appointment's own current time. This is not a rare slip; hand-composed datetimes are wrong roughly half the time.

**The law of this class: a value with a canonical machine source is COPIED by code, never composed by the model. The model's arithmetic is a hint for disambiguation — the words exchanged in the thread are the contract, and anything that can't be resolved to an offered option is refused, not guessed.**

| ID | Symptom | Guard now in place | New-account check |
|---|---|---|---|
| P-1 | Lead agreed to "3 to 6pm", booking landed at 8:00 PM; confirmation rendered the wrong hour faithfully | `resolveAgreedSlot` gates EVERY booking on every channel (was sales-only): exact ±60s match to a tool-offered ISO → that ISO verbatim; else the thread's own words pick the slot (window labels like "3 to 6pm" parse to their START; "12 to 3pm" can't be hijacked by its end; narrowed by month-day → weekday → the model's calendar day); else same-day-unique; else SMS/Messenger REFUSE and re-ask ("Almost there — which of the times I offered works best?"). Voice snaps from the call transcript and proceeds+flags when unresolvable (dead air is worse on a call). A visit booking with NO offered slots on file refuses outright — the model must run `find_available_slots` first. The snapped ISO re-runs the slot→tech map lookup so the corrected time inherits the right technician | Battery item 33. |
| P-2 | The invented time sat outside every arrival window yet passed the tech audit (anchored day + no ±2h conflict) | Business-hours bound in `techCanTakeBooking`: when enabled windows exist, ANY time outside all of them is rejected (tool-offered escape only; window-less accounts keep the legacy ±2h rule) | Battery item 33 — the 8 PM probe must return false; a real window start on the same anchored day must return true. |
| P-3 | `reschedule_appointment` wrote the model's raw datetime with no gate at all; observed live in testing: a "switch to 12 to 3pm" reschedule aimed at the slot the lead ALREADY held (self-move) | The same `resolveAgreedSlot` ladder gates moves (with C7 naive-datetime normalization in the lead's zone first); a move that resolves to the CURRENT time is a self-move tell — re-resolve against the OTHER offered slots, else refuse with "Happy to move it! Which of the times I offered works best?" | Battery item 33 — the reschedule turn must land on exactly the other offered ISO with `rescheduled_from` recorded. |

Residual, accepted: if the model's composed value coincidentally EQUALS a different offered slot exactly, the exact-match rung books that slot — bounded to a real window the lead was shown, and the self-move tell covers the observed shape of this. Prompt layers (engine tool description + per-account RULE "EXACT SLOT TIMES, NEVER YOUR OWN MATH" + voice rule 21) reduce how often any rung below "exact" fires; only the code ladder is the promise.

### Q. The platform-supplied identity (the KTrin Brownskin name class — added Aug 12, 2026)

The same booking carried a second defect: it reached HCP under the customer name **"KTrin Brownskin"** — her Facebook display name, an alias. Nobody ever asked her name because nobody had to: Facebook AUTOFILLS lead-form name fields from the profile, the Messenger path reads the same profile, and the engine's NAME rule only fired when the file had NO name. The file always had one — it just wasn't hers. The office spotted it at 9 PM ("can you provide your name for the job order?" → "Katrina Lewis") and renamed the HCP customer by hand; the CRM kept the alias until the fix.

**The law of this class: platform-supplied identity is not customer-confirmed identity. Anything auto-filled (Facebook profile names today; any future prefilled field) carries a provenance stamp and stays UNVERIFIED until the customer states it in their own words — and bookings ride only on verified identity. "A name is on file" and "we know who we're booking" are different facts.**

| ID | Symptom | Guard now in place | New-account check |
|---|---|---|---|
| Q-1 | HCP customer created under a Facebook alias; office chased the real name after hours | Both FB name paths (leadgen autofill + Messenger PSID profile) stamp `metadata.name_source="facebook_profile"`; a visit booking is HELD with the deterministic corrective "Perfect. And what's your full name for the booking?" until the name is verified; `update_lead_details` marks a name verified ONLY when it appears in the customer's own message that turn (the model cannot "verify" the alias it sees on file) → `name_verified=true`, `name_source="customer_stated"`; the recap, the CRM, and the HCP customer all carry the stated name. Chat greetings keep the profile first name — the BOOKING identity is what must be real. Prompt layers: lead-file marker on unverified names, engine rule 2b clause, per-account RULE "FULL NAME FOR THE BOOKING", voice rule 20 | Battery item 34. Onboarding: backfill `name_source="facebook_profile"` on existing FB leads (293 stamped on the origin account). |
...
33. ☐ **Agreed-time contract proof** (P-1..P-3) — seed offered slots + a recap thread, then drive a real-model consent turn and a reschedule turn. Assert: the appointment lands at EXACTLY an offered ISO with the slot-mapped tech (never the model's own datetime — in our battery the model composed it wrong in 4 of 5 runs); a never-offered day with no thread signal REFUSES with a re-ask instead of inserting; `techCanTakeBooking` rejects any time outside the enabled windows (the 8 PM test) while a real window start on an anchored day still passes; the voice `executeTool` path snaps the same wrong ISO using the call transcript.
34. ☐ **Facebook-name verification proof** (Q-1) — a lead with `name_source="facebook_profile"`: consent must NOT book; the reply asks "And what's your full name for the booking?"; the customer's stated name saves with `name_verified=true` + `name_source="customer_stated"`; the next consent books under the real name (which is what the HCP customer gets). Regression: a webhook/GHL lead with a customer-typed name books on first consent with NO name question.
...
- **The UI leg of the same law (Aug 12 2026): every office-facing `toLocale*String` call passes an explicit `timeZone`.** A 3:00 PM Chicago job stored as 20:00 UTC displayed as "11:00 PM" to the owner's UTC+3 browser — data correct, clock wrong. No dashboard, calendar, list, or tech view may format a timestamp with the viewer's browser default; company tz comes from `ai_agent_config.timezone` threaded as a prop. Week-grid columns anchor at **local noon** (never midnight), so each column moment stays on the same calendar day in both the viewer's and the company's timezone (±12h) — midnight anchors bucketed the whole grid one day off for viewers east of the company. "Today"/"past"/now-line math compares company-tz day keys, not `toDateString()`. Audit check: `grep -rn "toLocaleTimeString\|toLocaleDateString\|toLocaleString" app components | grep -v timeZone` on date/time (not money) call sites must return nothing.
...
- Every outbound message on a written channel passes through `sanitizeAgentText` (no dash punctuation, no asterisks, **no colons as punctuation** — clock times, URLs and emoticons survive; ranges as "to"). Prompt rules alone were probabilistic; this makes the style guarantee absolute. Voice is unaffected.
- **The Messenger questionnaire opener's only job is a REPLY** (owner decision Aug 2026, refined from a 49-thread opener study): the canonical shape — a CONCEPT to adapt, never a script — is a first-name intro ("Hey Gina, it's Linda with Top Air!" — name intros were the top reply lever in the data at ~92%), one specific answer echoed and normalized ("most homes we open up have never had it done, you're in good company"), then ONE question answerable instantly from memory ("How long have you guys been in the house?" / pets) — never a lead-in like "quick question", never something requiring recall or inspection. Never the address, never "ready to schedule?", never a price in message one (zero-reply openers in the study were all commitment-asks). When price enters later it starts from the $89 the lead came in holding and EARNS the $189 difference via the return-ducts mechanism — never quote $189 as if the $89 they saw doesn't exist. The address ask belongs at slot acceptance, where the booking gate forces it anyway.
