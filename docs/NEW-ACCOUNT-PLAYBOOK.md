...
- **The Messenger questionnaire opener's only job is a REPLY** (owner decision Aug 2026): first message references one specific answer, normalizes it, and asks ONE five-second diagnostic question about their home. Never the address, never "ready to schedule?", never a price in message one — those live in later messages. The address ask belongs at slot acceptance, where the booking gate forces it anyway.
