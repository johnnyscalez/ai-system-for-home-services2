...
- ☐ **Meta inbox automations count as "the office"**: pages often run Meta's own question flows (the intake questionnaire) — those page-side messages make a thread human-owned for the guard, on purpose. Turn those automations off if the AI should own new conversations.
